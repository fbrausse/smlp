        ...
        for i in test_id_list:
            ...
            if execute_test:
                ...
                for file in new_files:
                    ...
                    rds_file = new_file.endswith('.Rds')  # rds file needs to be translated to txt for comparing
                    rds_file_path = None  # .rds file full path
                    master_file_rds_path = None  # .rds master path
                    txt_file = False
                    if path.exists(master_file):
                        if rds_file:  # deals with rds files (model)
                            rds_file_path = new_file.replace('\\\\', '\\').replace('\\', '/')
                            file = file.replace('.Rds', '.txt')
                            new_file = new_file.replace('.Rds', '.txt')
                            model_file = True
                            master_file_rds_path = master_file
                            master_file = master_file.replace('.Rds', '.txt')
                            Popen("{Rscript} -e \'readRDS(\"{file_name}\")\' > {new_file}".format(Rscript=Rscript,
                                                                                                  file_name=rds_file_path,
                                                                                                  new_file=new_file)
                                  , shell=True).wait()

                            '''Check if txt file already exists, if not create one from .rds file'''
                            if not path.exists(master_file):
                                Popen("{Rscript} -e \'readRDS(\"{file_name}\")\' > {new_file}".format(Rscript=Rscript,
                                                                                                      file_name=master_file_rds_path,
                                                                                                      new_file=master_file),
                                      shell=True).wait()
                        elif new_file.endswith('.txt') and not config_file :
                            txt_file = True
                        ...
                        if (new_file.endswith('.csv') or new_file.endswith('.txt') or  new_file.endswith('.html') or new_file.endswith('.json')) and not file_name in files_to_ignore_from_diff:
                                ...
                                if not comapre_files(new_file, master_file):
                                    ...
                                    if user_input in {'yes', 'y'}:
                                        if model_file or config_file:
                                            if rds_file:
                                                copyfile(rds_file_path, master_file_rds_path)
                                                copyfile(new_file, master_file)
                                                copyfile(rds_file_path, path.join(models_path, file_name))
                                            else:
                                                copyfile(new_file, master_file)
                        ...
                        if file.endswith("eva_error.txt"):
