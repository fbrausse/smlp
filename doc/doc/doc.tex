...
On the other hand, if the $\assert(p,x,y)$ does not have a $\theta$-stable counter-example, then it is still possible 
...
If the strengthened assertion $\assert'(p,x,y)$ has a $\theta_{r'}$-stable counter-example $x'$ for a configuration $p'$ on the 
model, then $p', x'$ can be used to find a counter-example to the original assertion $\assert(p, x, y)$ on the system in the same 
