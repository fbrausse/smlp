...
# global data parameter defaults for SMLP
...
data_params_dict = {
    ...
    'split-test': {'abbr':'split', 'default':DEF_SPLIT_TEST, 'type':float,
        'help':'Fraction in (0,1] of data samples to split from training data' +
            ...
            ' both for training and testing [default: {}]'.format(str(DEF_SPLIT_TEST))}, 
    ...
    'sample_weights_coef': {'abbr':'sw_coef', 'default':DEF_SAMPLE_WEIGHTS, 'type':float,
...
class DataCommon:
    ...
    def _encode_categorical(self, df, levels_dict):
        ...
        if len(unseen_levels_dict.keys()) > 0:
            ...
            self._data_logger.error(err_msg)
...
'''        
# Compute sample weights for given response based on its values, resp_vals,
# and based on constant sw_coef -- the value for command line option sw_coef.
# The weights are returned as an np.array.
def sample_weights_per_response(resp_vals, sw_coef):
