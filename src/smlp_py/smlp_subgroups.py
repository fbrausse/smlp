...
class SubgroupDiscovery:
        ...
        if rng_tuple['origin'] != self._ORIGIN_NUM and rng_tuple['origin'] != self._ORIGIN_BIN:
            ...
            if rng_tuple['hi'] < rng_tuple['lo']:
                ...
                if rng_tuple['origin'] == self._ORIGIN_BIN:
                    ...
                    rng_ft = ((ft <= rng_tuple['hi']) & (ft >= rng_tuple['lo'])).astype(int)
        return rng_ft
    ...
    def feat_resp2num_freqs(self, feat_name, feat, resp, non_range_feat, pos_value_name):
        ...
        resp_min = min(resp)
        resp01 = (resp - resp_min)/(max(resp)-resp_min)
        resp10 = 1 - resp01 
    ...
    def feat_resp2pos_sample_indices(self, feat, resp, rownms, pos_value_name, neg_value_name, 
        ...
        pos_val = pos_value_name if isPos else neg_value_name
        feat_val = 1 if inVsOut else 0
        ...
        subset_cond_list = (feat == feat_val) & (resp == pos_val)
        class_samples_in = [str(rownms[i]) for i in range(len(rownms)) if subset_cond_list[i]]
        ...
        if expand:
            ...
            feat_class_bin_col = [e in class_samples_in for e in class_samples] 
    ...
    def _psg_rules_to_ranking_df(self, resp_name:str, resp_df:pd.DataFrame,
        ...
        def single_range_rule_to_row(r):
            ...
            if compute_full_fs_ranking_df:
                ...
                if col_type == self._ORIGIN_NUM:
                    row = (rng_tup['name'], str(rng_tup['lo']) + ':' + str(rng_tup['hi']), '1:1', 1)
                else:
                    row = (rng_tup['name'], psg_range_tuple[1], 'NA:NA', 'NA')
        ...
        for i, r in enumerate(rules):
            ...
            for j in [0,1,2]: # indices in 
                if j >= r_dim:
                    row_i = (row_i) + emty_range
                    continue
        
                if r_dim == 1:
                    row_dict = single_range_rule_to_row(str(r))
                    row = row_dict['row']
                    name_i = rng_nm = row_dict['rng_nm']
                    row_i = (row_i) + row if not row_i is None else row
                    sel_range_names.append(rng_nm)
                    
                if r_dim == 2: 
                    pr = psg_pairs_map_df[str(r)]
                    sel = pr[j]
                    row_dict = single_range_rule_to_row(sel)
                    rng_nm = row_dict['rng_nm']
                    row = row_dict['row']
                    if not row_i is None:
                        row_i = (row_i) + row
                        assert not name_i is None
                        name_i = name_i + self._RANGES_SEP + rng_nm
                        rng_feat_i = rng_feat_i & levels_df[rng_nm]
                    else:
                        row_i = row
                        name_i = rng_nm
                ...
                if r_dim == 3:
                    ...
                    if j == 2:
                        ...
                        for m in [0,1]:
                            sel_m = tr[m]
                            row_dict_m = single_range_rule_to_row(sel_m)
                            rng_nm_m = row_dict_m['rng_nm']
                            rng_tup_m = row_dict_m['rng_tup']
                            row = row_dict_m['row']
                            ...
                            if origin == self._ORIGIN_NUM:
                                ...
                                #lvl = unique_level_to_level(rng_nm_m, ranges_map)
                                cond = str(col) == lvl
                            else:
                                raise Exception('Unknown tuple origin in function psg_rules2ranking_df')
                            feat_df_cond = feat_df_cond[cond]
            ...
            if not skip_this_range:  
                ...
                row_dict = dict(zip(colnames, row_i))
    ...
    def _fs_multi_response_ensemble_summary(self, full_fs_list, extracted_resp_feat_list_full,
        ...
        resp_names = list(full_fs_list.keys())
        for resp_name, single_fs in full_fs_list.items():
            assert resp_name == single_fs['resp_name'];
            resp = single_fs['resp']
            ranked_fs_summary_df = single_fs['ranked_fs_summary_df']
