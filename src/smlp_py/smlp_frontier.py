...
#         return structured_array
    ...
    def local_minimum(self, df, feat_names, resp_names, objv_names, func_intervals):
        ...
        for i, (lower, upper) in zip(df.index, zip(lower_bounds, upper_bounds)):
            ...
            min_values = df.loc[mask, objv_names].min()
            df_min.loc[i, objv_names] = min_values; 
        
        df_min = self.drop_rows_with_nan_in_columns(df_min, objv_names)
        return df_min
    ...
    def find_pareto_front(self, df, knobs, objectives, precision=None):
        ...
        df_groupby_one = pareto_df.drop_duplicates(subset=objectives)
        ...
        pareto_groupby_one = df_groupby_one[is_pareto_optimal]
        ...
        pareto_front = pareto_df.loc[pareto_front_indices]
    ...
    def select_pareto_frontier(self, X:pd.DataFrame, y:pd.DataFrame, model_features_dict:dict, 
        ...
        df_alpha = self.filter_by_expression(df, alph_expr)
        ...
        df_objv = self.compute_objectives_columns(df_alpha, objv_names, objv_exprs)
        ...
        df_stable_min_shape = df_stable_min.shape
        ...
        pareto_frontier = self.find_pareto_front(df_beta, knobs, objv_names)
