...
class SmlpOptimize:
    ...
    def compute_objectives_bounds(self, X:pd.DataFrame, y:pd.DataFrame, objv_names:list[str], objv_exprs:list[str]):
        df_resp_feat = pd.concat([X,y], axis=1)
        objv_bounds_dict = {}
        ...
        def eval_objv(row):
            ...
            res_row = eval(objv_cond, {}, eval_env)
            return res_row

        for i, (objv_name, objv_cond) in enumerate(zip(objv_names, objv_exprs)):
            objv_series = df_resp_feat.apply(lambda row : eval_objv(row), axis=1)
    ...
    def bin_opt_max(self, smlp_domain:core.domain, model_full_term_dict:dict, phi_cond:core.form2, objv_name:str, objv_expr:str, 
            objv_term:core.term2, l0:float, u0:float, epsilon:float, solver_logic:str):
        ...
        while True:
            ...
            quer_form = self._smlpTermsInst.smlp_ge(objv_term, core.Cnst(T))
            ...
            res = self._queryInst._modelTermsInst.smlp_solver_check(max_solver, 'max_ca', self._queryInst._lemma_precision)
            ...
            if self._modelTermsInst.solver_status_unsat(res):
                ...
                l = T
                p_cand = self._modelTermsInst.get_solver_knobs_model(res)
    ...
    def bin_opt_min(self, smlp_domain:core.domain, model_full_term_dict:dict, phi_cond:core.form2, objv_name:str, objv_expr:str, 
            objv_term:core.term2, l0:float, u0:float, epsilon:float, solver_logic:str):
    ...
    def optimize_single_objective_eager(self, resp_names:list[str], model_full_term_dict:dict, objv_name:str, objv_expr:str, objv_term:core.term2,
        ...
        phi_cand = self._smlpTermsInst.smlp_and_multi([eta, alpha, beta])
        ...
        while True:
            ...
            if l_stab + epsilon > u_cand:
                ...
                break
            phi_cond = self._smlpTermsInst.smlp_and(phi_cand, L)
            ...
            if self._modelTermsInst.solver_status_sat(res):
                ...
                objv_witn_val_term = core.subst(objv_term, stable_witness_terms)
    ...
    def optimize_single_objective(self, resp_names:list[str], model_full_term_dict:dict, objv_name:str, objv_expr:str, objv_term:core.term2,
        ...
        while True:
            if u == np.inf:
                ...
                T = (l + u) / 2
            #quer_form = objv_term > core.Cnst(T)
            #quer_form = objv_term >= core.Cnst(T) 
            ...
            if stable_witness_status == 'UNSAT':
                ...
                if len(P) == 0:
                    ...
                    if objv_expr is not None:
                        orig_objv_const_term = core.subst(objectives_unscaler_terms_dict[orig_objv_name], #objv_term, 
                    ...
                    if l not in [np.inf, -np.inf]:
                        ...
                        stable_witness_terms['threshold_lo_scaled'] = core.Cnst(l)
                    ...
                    if u not in [np.inf, -np.inf]:
                        ...
                        stable_witness_terms['threshold_up_scaled'] = core.Cnst(u)
        ...
        if u not in [np.inf, -np.inf]:
            orig_max = objv_bounds[orig_objv_name]['max']
            orig_min = objv_bounds[orig_objv_name]['min']
            if scale_objectives:
                P[-1]['threshold_up_scaled'] = u
                P[-1]['threshold_up'] = orig_min + u * (orig_max - orig_min)
            else:
                P[-1]['threshold_up'] = u
    ...
    def optimize_single_objectives(self, feat_names:list, resp_names:list, #X:pd.DataFrame, y:pd.DataFrame, 
            model_full_term_dict:dict, objv_names:list, objv_exprs:list, objv_bounds_dict:dict, alpha:core.form2, beta:core.form2, 
            eta:core.form2, theta_radii_dict:dict, epsilon:float, smlp_domain:core.domain, delta:float, solver_logic:str, strategy:str,  scale_objv:bool,  
    ...
    def active_objectives_max_min_bounds(self, resp_names:list[str], model_full_term_dict:dict, objv_terms_dict:dict, t:list[float],
        ...
        for j, (objv_name, objv_term) in enumerate(objv_terms_dict.items()):
            if t[j] is not None:
                eta_F_t = self._smlpTermsInst.smlp_and(eta_F_t, objv_term > core.Cnst(t[j]))
            else:
                min_name = min_name + '_' + objv_name if min_name != '' else objv_name
                if min_objs is not None:
                    min_objs = core.Ite(objv_term < min_objs, objv_term, min_objs)
                else:
                    min_objs = objv_term
        ...
        c_up = r['threshold_up'] if 'threshold_up' in r else np.inf
        c_lo = r['threshold_lo']
        assert c_lo != np.inf
    ...
    def prog_dict_to_df(self):
        prog_dict = self.best_config_dict
        row_names = []
        new_prog_dict = {}
        prog_df = None
    ...
    def report_current_thresholds(self, s, witness_vals_dict, objv_bounds_dict, objv_names, objv_exprs, 
        ...
        if scale_objv:
            ...
            s_scaled = [self.scale_constant_val(objv_bounds_dict, objv_name, b) 
                for (objv_name, b) in zip(objv_names,s)]
        s_scaled_str = ["%.6f" % e for e in s_scaled]
        s_origin_str = ["%.6f" % e for e in s_origin]
            ...
            for key, val in witness_vals_dict.items():
                ...
                if key in self.resp_names and self.syst_expr_dict is not None:
                    if key in self.syst_expr_dict.keys():
                        ...
                        system_val = eval(self.syst_expr_dict[key], {},  witness_vals_dict)
    ...
    def update_optimization_reports(self, stable_witness_terms, l, u, call_info, iter_count, scale_objectives, 
        ...
        if call_info is not None and call_info['update_thresholds'] and update_progress_report:
            witness_vals_dict = self._smlpTermsInst.witness_term_to_const(stable_witness_terms, sat_approx,  
                sat_precision)
            del witness_vals_dict[objv_name]
        ...
        if scale_objectives: 
            ...
            #assert False
            if l not in [np.inf, -np.inf]:
                stable_witness_terms['threshold_lo'] = core.Cnst(l)
            if u not in [np.inf, -np.inf]:
                stable_witness_terms['threshold_up'] = core.Cnst(u)
        stable_witness_terms['max_in_data'] = core.Cnst(objv_bounds[orig_objv_name]['max'])
        stable_witness_terms['min_in_data'] = core.Cnst(objv_bounds[orig_objv_name]['min'])
        stable_witness_vals = self._smlpTermsInst.witness_term_to_const(
            stable_witness_terms, sat_approx, sat_precision)
    ...
    def optimize_pareto_objectives(self, feat_names:list[str], resp_names:list[str], 
            model_full_term_dict:dict, objv_names:list, objv_exprs:list, objv_bounds_dict:dict, alpha:core.form2, 
            beta:core.form2, eta:core.form2, theta_radii_dict:dict,
            epsilon:float, smlp_domain:core.domain, delta:float, solver_logic:str, strategy:str, scale_objv:bool, data_scaler:str, 
    ...
    def check_synthesis_feasibility(self, feasibility:bool, objv_names:list[str], objv_exprs:list[str], objv_bounds_dict:dict, 
            scale_objv:bool, feat_names:list[str], resp_names:list[str], model_full_term_dict:dict, beta:core.form2, 
            domain:core.domain, eta:core.form2, alpha:core.form2, theta_radii_dict:dict, delta:float, solver_logic:str, 
    ...
    def smlp_optimize(self, syst_expr_dict:dict, algo:str, model:dict, X:pd.DataFrame, y:pd.DataFrame, model_features_dict:dict, 
        ...
        objv_bounds_dict = self.compute_objectives_bounds(X, y, objv_names, objv_exprs)
    ...
    def smlp_optsyn(self, syst_expr_dict:dict, algo, model, X:pd.DataFrame, y:pd.DataFrame, model_features_dict:dict, feat_names:list[str], resp_names:list[str], 
        ...
        if pareto:
            self.optimize_pareto_objectives(feat_names, resp_names, model_full_term_dict,
                objv_names, objv_exprs, objv_bounds_dict, alpha, beta, eta, theta_radii_dict, 
                epsilon, domain, delta, solver_logic, strategy, scale_objv, data_scaler,
                sat_approx=True, sat_precision=64, save_trace=False)            
        else:
            self.optimize_single_objectives(feat_names, resp_names, model_full_term_dict, 
                objv_names, objv_exprs, objv_bounds_dict, alpha, beta, eta, theta_radii_dict, 
                epsilon, domain, delta, solver_logic, strategy, scale_objv, data_scaler, 
                sat_approx=True, sat_precision=64, save_trace=False)
